...
export const CardClassroom: FC<IProps> = ({ classroom }) => {
    return (
        <Link to={`${Screen.Classrooms}/${classroom.nameClass}`}>
            <button
                className={`card-classroom border-2  border-b-4 border-darkGray focus:outline-none cursor-pointer relative rounded-xl flex items-center justify-center ss:px-6 ss:py-6 p-3`}
                key={classroom.id}
            >
